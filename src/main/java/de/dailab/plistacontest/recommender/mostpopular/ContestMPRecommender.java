...
public class ContestMPRecommender implements ContestRecommender {
	...
	public void init() {
		...
		// load Stats
		this.stats = Desirializer.<Stats> deserialize("MPRecommender_" + "stats.ser");
		if (this.stats == null) {
			this.stats = new Stats("MPRecommender");
		}
	}
	...
	public List<ContestItem> recommend(final Impression _impression) {
		...
		try {
			...
			tmpRec = this.domainRecommender.get(_impression.getDomain());
		...
		if (this.counter.get(_impression.getDomain()) >= this.impressionCount) {
			...
			new Thread() {
				...
				public void run() {
					...
					Serializer.serialize(stats, "MPRecommender" + "_stats.ser");
				}
	...
	public void feedback(final Feedback _feedback) {
		try {
			...
			if (_feedback.getTeamId() == Integer.parseInt(this.properties.getProperty("plista.teamId"))
				...
				Serializer.serialize(this.stats, "MPRecommender" + "_stats.ser");
			}
